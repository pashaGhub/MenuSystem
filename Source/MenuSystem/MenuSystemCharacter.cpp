	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	if (OnlineSubsystem)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, FString::Printf(TEXT("Found subsystem: %s"), *OnlineSubsystem->GetSubsystemName().ToString()));
		}
	}
	else
	{
		UE_LOG(LogTemplateCharacter, Warning, TEXT("OnlineSubsystem not found!"));
	} 
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, FString::Printf(TEXT("Created session: %s"), *SessionName.ToString()));
		}
...
void AMenuSystemCharacter::OnFindSessionsCompleteDelegate(bool bWasSuccessful)
{
	if (!OnlineSessionInterface.IsValid())
	{
		return;
	...
	for (auto SearchResult : SessionSearch->SearchResults)
	{
		...
		if (MatchType == FString("FreeForAll"))
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Turquoise, FString::Printf(TEXT("MatchType: %s"), *MatchType));
			}
...
void AMenuSystemCharacter::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Yellow, FString::Printf(TEXT("OnJoinSessionComplete cb starts...")));
	}
	if (!OnlineSessionInterface.IsValid())
	{
		return;
	...
	else
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString(TEXT("Failed to join session!")));
		}
	}
}
